...
### Pada data_202301, kolom started_at dan ended_at bertipe datetime. Ubah menjadi chr.
...
summary_data <- all_data %>% 
  mutate(weekday = wday(started_at, label = TRUE)) %>%  #creates weekday field using wday() from started_at column
  group_by(member_casual, weekday) %>%  #groups by usertype and weekday
  summarise(number_of_rides = n(),  #calculates the number of rides by counting the number of rows in each group
            average_duration = mean(ride_length),
            .groups = "keep") %>%     # calculates the average duration of ride_length column in minutes
  arrange(member_casual, weekday)   # sorts
summary_data

#=====VISUALISASI DATA (share)=====
# Membuat pie chart untuk melihat presentase pengguna
summary_user %>% 
  ggplot(aes(x = "", y = count, fill = member_casual)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar(theta = "y") +
  labs(title = "Jumlah Pengendara Biasa (Casual) vs Anggota (Member)",
...
# Membuat bar chart untuk banyak perjalanan yang dilakukan pengguna
summary_data  %>% 
...
summary_data %>% 
  ...
  geom_col(position = "dodge") +
  labs(title = "Distribusi Rata-rata Durasi Perjalanan Pengguna",
       subtitle = "per Hari dari Bulan Januari sampai Agustus 2024",
       x = "Hari",
       y = "Rata-rata Durasi Perjalanan (menit)",
       fill = "Rider Type") +
